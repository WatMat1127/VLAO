...
class LinkJOB:
    def __init__(self, path):
        ...
        with open(path, mode="r") as f:
            ...
            coordinate_i = [
                i
                for i, line in enumerate(fdat_linkjob_list)
                if "CURRENT COORDINATE" in line
            ]
            energy_i = [
                i for i, line in enumerate(fdat_linkjob_list) if "ENERGY" in line
            ]
            gradient_i = [
                i for i, line in enumerate(fdat_linkjob_list) if "GRADIENT" in line
            ]
            hessian_i = [
                i for i, line in enumerate(fdat_linkjob_list) if "HESSIAN" in line
            ]
            hessian_end_i = [
                i
                for i, line in enumerate(fdat_linkjob_list)
            ...
            for iatom in range(natom):
                element_iatom = fdat_linkjob_list[coordinate_i[0] + 1 + iatom].split()[
                    0
                ]
                x_iatom = (
                    float(fdat_linkjob_list[coordinate_i[0] + 1 + iatom].split()[1])
                    * unit_ang2au()
                )
                y_iatom = (
                    float(fdat_linkjob_list[coordinate_i[0] + 1 + iatom].split()[2])
                    * unit_ang2au()
                )
                z_iatom = (
                    float(fdat_linkjob_list[coordinate_i[0] + 1 + iatom].split()[3])
                    * unit_ang2au()
                )
                coordinate_list.append(
                    [element_iatom, np.array([x_iatom, y_iatom, z_iatom])]
                )
            ...
            for jatom in range(self.natom_active):
                for idim in range(3):
                    for jdim in range(3):
                        if iatom == jatom:
                            if idim >= jdim:
                                self.hess[iatom][jatom][idim][jdim] += add_hess[iatom][
                                    jatom
                                ][idim][jdim]
                        elif iatom > jatom:
                            self.hess[iatom][jatom][idim][jdim] += add_hess[iatom][
    ...
    def write_LinkJOB(self, path_LinkJOB_write):
        ...
        for i in range(3 * natom % 5):
            hessian_write.append(
                tmp_list_2[5 * (3 * natom // 5) + i][5 * (3 * natom // 5) :]
            )
        ...
        fdat_linkjob_list_write[self.energy_index] = (
            f"ENERGY =   {self.ene: 15.12f}    0.000000000000    0.000000000000"
        )
        for iatom in range(natom):
            fdat_linkjob_list_write[self.grad_index + 3 * iatom + 1] = (
                f"{self.grad[iatom][0]: 11.9f}"
            )
            fdat_linkjob_list_write[self.grad_index + 3 * iatom + 2] = (
                f"{self.grad[iatom][1]: 11.9f}"
            )
            fdat_linkjob_list_write[self.grad_index + 3 * iatom + 3] = (
                f"{self.grad[iatom][2]: 11.9f}"
            )
        for iline in range(self.hessian_end_index - self.hessian_index - 1):
            ...
            fdat_linkjob_list_write[self.hessian_index + 1 + iline] = (
                "   " + "\t   ".join(hessian_write[iline])
            )
...
class ComFile:
    def __init__(self, path):
        with open(path, mode="r") as f:
            ...
            frozen_index = [
                i for i, line in enumerate(fdat_com_list) if "frozen" in line.lower()
            ]
            end_index = [
                i for i, line in enumerate(fdat_com_list) if "options" in line.lower()
            ]
            if len(frozen_index) == 0:
                ...
                for iatom in range(len(frosen_struct_list)):
                    ...
                    x_iatom = (
                        float(frosen_struct_list[iatom].split()[1]) * unit_ang2au()
                    )
                    y_iatom = (
                        float(frosen_struct_list[iatom].split()[2]) * unit_ang2au()
                    )
                    z_iatom = (
                        float(frosen_struct_list[iatom].split()[3]) * unit_ang2au()
                    )
                    coordinate_list.append(
                        [element_iatom, np.array([x_iatom, y_iatom, z_iatom])]
                    )
...
class ParamFile:
    def __init__(self, path, natom_all):
        with open(path, mode="r") as f:
            ...
            ligand_param_index = [
                i for i, line in enumerate(fdat_param_list) if "LIGLIGLIGLIG" in line
            ]

            ligand_atoms = []
            for iligand in range(nligand):
                atom_number_P = (
                    int(fdat_param_list[ligand_param_index[iligand] + 1].split("=")[1])
                    - 1
                )
                atom_number_xlist = [
                    int(i) - 1
                    for i in fdat_param_list[ligand_param_index[iligand] + 2]
                    .split("=")[1]
                    .split(",")
                ]
                atom_number_offtgt = [
                    int(i) - 1
                    for i in fdat_param_list[ligand_param_index[iligand] + 3]
                    .split("=")[1]
                    .split(",")
                ]
                ligand_atoms.append(
                    [atom_number_P, atom_number_xlist, atom_number_offtgt]
                )
            ...
            for iligand in range(nligand):
                ...
                tmp_info = fdat_param_list[
                    ligand_param_index[iligand] + 1 : ligand_param_index[iligand + 1]
                ]
                keep_idx = [
                    i for i, line in enumerate(tmp_info) if "keeppot" in line.lower()
                ]
                keeppyr_idx = [
                    i
                    for i, line in enumerate(tmp_info)
                    if "keepangle" in line.lower() or "keeppyr" in line.lower()
                ]
                LJ_asym_ell_idx = [
                    i
                    for i, line in enumerate(tmp_info)
                    if "ovoid_ljpot" in line.lower() or "ljpot_asym_ell" in line.lower()
                ]
                ...
                for ipot in range(nkeeppyr):
                    ikeeppyr_info = {}
                    keeppyr_tmp_list = (
                        tmp_info[keeppyr_idx[ipot]].split("=")[1].split(";")
                    )
                ...
                for ipot in range(nLJ_asym_ell):
                    iLJ_asym_ell_info = {}
                    LJ_asym_ell_tmp_list = (
                        tmp_info[LJ_asym_ell_idx[ipot]].split("=")[1].split(";")
                    )
                    atom_number_xi = int(LJ_asym_ell_tmp_list[0]) - 1

                    epsilon = self.get_param(
                        LJ_asym_ell_tmp_list[1], unit_kcal2hartree()
                    )
                    ...
                    for iatom in range(natom_all):
                        if (
                            iatom in atom_number_offtgt
                            or iatom == atom_number_P
                            or iatom in atom_number_xlist
                        ):
                            pass
                        else:
...
class PhiLog:
    def __init__(self, path):
        with open(path, mode="r") as f:
            ...
            itr_index = [
                i for i, line in enumerate(fdat_phi_log_list) if "-----" in line
            ]
