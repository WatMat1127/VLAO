...
def VL_param_grad(xyz, param, param_tag, MM_param_list, path_phi_log):
    ...
    for itag in dat_keep_info_tag:
        ...
        tensor_list.append(
            torch.tensor(
                param.keep_info[term_0[0]][term_0[1]][term_0[2]], requires_grad=True
            )
        )
        tensor_tag.append(itag)

    if len(tensor_tag) >= 1:
        grad_keeppot = torch.func.jacfwd(calc_keep_pot_pgrad, argnums=2)(
            xyz, param_tag.keep_info, tensor_list, tensor_tag
        )
    ...
    for itag in dat_keeppyr_info_tag:
        ...
        tensor_list.append(
            torch.tensor(
                param.keeppyr_info[term_0[0]][term_0[1]][term_0[2]], requires_grad=True
            )
        )
        tensor_tag.append(itag)

    if len(tensor_tag) >= 1:
        grad_keeppyrpot = torch.func.jacfwd(calc_keep_pyr_pot_pgrad, argnums=2)(
            xyz, param_tag.keeppyr_info, tensor_list, tensor_tag
        )
        for itag in range(len(tensor_tag)):
    ...
    for itag in dat_LJ_asym_ell_info_tag:
        ...
        tensor_list.append(
            torch.tensor(
                param.LJ_asym_ell_info[term_0[0]][term_0[1]][term_0[2]],
                requires_grad=True,
            )
        )
        tensor_tag.append(itag)
    ...
    if len(tensor_tag) >= 1:
        ...
        grad_LJ_pot_asym_ell = torch.func.jacfwd(calc_LJ_asym_ell_pot_pgrad, argnums=4)(
            xyz,
            phi_list,
            MM_param_list,
            param_tag.LJ_asym_ell_info,
            tensor_list,
            tensor_tag,
        )
        for itag in range(len(tensor_tag)):
    ...
    with open(param.path + "_grad", mode="w") as g:
        for itag in dat_param_grad:
            if itag in dat_convert.keys():
                g.write(
                    "{0:<15} {1}\n".format(
                        itag, dat_param_grad[itag] * dat_convert[itag]
                    )
                )
            else:
                g.write(
