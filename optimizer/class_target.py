...
class Target:
    ...
    def make_input_files(self, dat_info):
        ...
        if os.path.isfile(ftop_new + ".com"):
            ...
            if nstep >= 1:
                ...
                log_file_best = class_general.LogFile(
                    self.fn_target + "_step{}".format(self.nstep_best) + ".log"
                )
                ...
                subprocess.run(
                    "cp {0}.phi_log {1}.phi_log".format(ftop_best, ftop_new), shell=True
                )
    ...
    def copy_result(self, nstep, istep):
        ...
        subprocess.run(
            "cp {0}.phi_log {1}.phi_log".format(fn_istep, fn_nstep), shell=True
        )
        subprocess.run(
            "cp {0}_MO.rrm {1}_MO.rrm".format(fn_istep, fn_nstep), shell=True
        )
        subprocess.run(
            "cp {0}_LinkJOB.rrm {1}_LinkJOB.rrm".format(fn_istep, fn_nstep), shell=True
        )
        subprocess.run(
            "cp {0}_LinkJOB.rrm_old {1}_LinkJOB.rrm_old".format(fn_istep, fn_nstep),
            shell=True,
        )
        subprocess.run(
            "cp {0}_message_END.rrm {1}_message_END.rrm".format(fn_istep, fn_nstep),
            shell=True,
        )

        with open("analysis_copy.txt", mode="a") as g:
            g.write(
                "step {0} is copied as step {1}: {2}\n".format(
                    istep, nstep, self.fn_target
                )
            )
    ...
    def read_QM_struct_ene(self, nstep, ene_tag):
        ...
        path_message_linkerror = (
            self.fn_target + "_step{}_message_LinkERROR.rrm".format(nstep)
        )
        ...
        while wait_tag:
            if os.path.isfile(path_message_end):
                with open(path_log, mode="r") as f:
                    ...
                    struct_idx = [
                        i for i, line in enumerate(list) if "Optimized" in line
                    ]
                    ene_idx = [
                        i for i, line in enumerate(list) if "ENERGY    =" in line
                    ]
                ...
                os.path.isfile(path_message_stop)
                or os.path.isfile(path_message_error)
                or os.path.isfile(path_message_linkerror)
            ):
                print("Some error happened in QM calculation.", flush=True)
                print(path_log)
                exit()
    ...
    def calc_grad(self, dat_info):
        ...
        subprocess.run(
            "cp {0} {1}".format(self.fn_target + ".param", path_param_tag), shell=True
        )
        ...
        if os.path.isfile(path_param_grad):
            ...
            subprocess.run(
                "python {0} {1} param_grad=True".format(path_sterep, path_LinkJOB),
                shell=True,
            )
        grad = []
