...
def calc_f_val_grad(qm_ene_list, grad_list):
    A_l_ene = qm_ene_list[0]
    B_b_ene = qm_ene_list[1]

    A_l_rel_ene = (A_l_ene - B_b_ene) * 627.51
    ...
    sum_all = A_l_exp + B_b_exp
    sum_b = B_b_exp

    rr = sum_b / sum_all * 100
    f_val = (rr) * (-1)

    A_l_grad = grad_list[0] * 627.51
    B_b_grad = grad_list[1] * 627.51

    d_sum_all = -1 / (const_r * const_t) * (A_l_exp * A_l_grad + B_b_exp * B_b_grad)
